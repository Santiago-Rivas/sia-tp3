...
class NonLinearPerceptron(Perceptron):
    ...
    def __init__(self, learning_rate, data, expected_value, sigmoid_func, sigmoid_func_img, sigmoid_func_derivative):
		super().__init__(data, expected_value,learning_rate)

		self.expected_range = (np.min(self.expected_value), np.max(self.expected_value))
		self.sigmoid_func = sigmoid_func
		self.sigmoid_func_img = sigmoid_func_img
		self.sigmoid_func_derivative = sigmoid_func_derivative
		self.scaled_expected_values = feature_scaling(self.expected_value, self.expected_range, self.sigmoid_func_img)
    ...
    def compute_deltas(self):
		...
		deltas = self.learning_rate * (output_errors * derivatives).reshape(-1, 1) * self.data

		return deltas

    def is_converged(self):
		expected_values_amplitude = np.max(self.scaled_expected_values) - np.min(self.scaled_expected_values)
